...
MODELA_PATH = "./models/model_hff_a_swa.checkpoint"
if __name__ == "__main__":

    smooth_diag = np.load("../resources/4DNFI643OYP9.rebinned.mcool.expected.res4000.npy")
    normmat = np.exp(smooth_diag[np.abs(np.arange(8000)[:, None] - np.arange(8000)[None, :])])

    t = Genomic2DFeatures(
        ["../resources/4DNFI643OYP9.rebinned.mcool::/resolutions/4000"],
        ["r4000"],
        (8000, 8000),
        cg=True,
    )
    ...
    denet_1_pt = nn.DataParallel(Decoder_1m())
    denet_1_pt_dict = denet_1_pt.state_dict()
    pretrained_dict = torch.load(MODELA_PATH)
    pretrained_dict_filtered = {key: pretrained_dict[key] for key in denet_1_pt_dict}
    denet_1_pt.load_state_dict(pretrained_dict_filtered)
    for param in denet_1_pt.parameters():
        param.requires_grad = False
